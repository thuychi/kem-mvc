...
class Receiver {
  ...
  getPayloadLength64 () {
    ...
    var buf = this.readBuffer(8), num = buf.readUInt32BE(0, true);
  ...
  dataMessage () {
    if (this.fin) {
      var messageLength = this.messageLength, fragments = this.fragments;
      ...
      if (this.opcode === 2) {
        ...
        this.onmessage(JSON.parse(data), { masked: this.masked, binary: true });
      } else {
        ...
        this.onmessage(JSON.parse(buf), { masked: this.masked });
      }
    }
